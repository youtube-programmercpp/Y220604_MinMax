...
/*【このサンプルのテーマ】
* C99を前提にする（これよりも古いバージョンの書き方はなるべく避ける）
* 関数を用意する
* 先頭の要素だけは特別扱いする
* 【説明】
* 入力データを受け付けるための関数として input、
* 出力データを表示するための関数として output を用意する。
* 先頭の要素の読み込みが失敗した場合、平均値、最大値、最小値を求めることはできない。
* というか、それができてしまったらバグなので、先頭の要素は特別扱いする。
* この扱いを for 文の中に入れてしまうと、初めに1回だけを行えば済む判定処理を
* １０回繰り返すことになってしまう…
*/
